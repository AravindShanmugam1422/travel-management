    ...
    <div class="card">
        ...
        {#if showPassengerForm}
            <form
                method="POST"
                action="?/addPassenger"
                use:enhance={() => async ({ update, result }) => { await update(); if (result.type === 'success') showToast('Passenger added successfully!'); showPassengerForm = false; }}
            >
                <div class="field-row">
                    <div class="field">
    ...
    <div class="card">
        ...
        {#if showTripForm}
            <form method="POST" action="?/addTrip" use:enhance>
                ...
                <button class="btn btn-primary" type="submit">Create & Open trip</button>
            </form>
        {/if}
